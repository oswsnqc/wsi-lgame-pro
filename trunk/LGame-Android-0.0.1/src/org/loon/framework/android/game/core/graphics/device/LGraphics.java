...
public final class LGraphics extends LGraphicsMath implements LTrans {
	...
	public static final int HCENTER = 0x00000001;		// 1
	public static final int VCENTER = 0x00000010;		// 2

	public static final int LEFT = 0x00000100;			// 4
	public static final int RIGHT = 0x00001000;			// 8
	public static final int TOP = 0x00010000;			// 16
	public static final int BOTTOM = 0x00100000;		// 32

	public static final int BASELINE = 0x01000000;		// 64

	public static final int SOLID = 0;
	public static final int DOTTED = 1;
	
	private final static android.graphics.DashPathEffect dashPathEffect = new android.graphics.DashPathEffect(new float[] { 5, 5 }, 0);
	 ...
	 * 设置Matrix
	 */
	public void setMatrix(float x1, float y1, float x2, float y2, float x3, float y3) {
		tmp_matrix.reset();
	...
	public Stroke getStroke() {
		...
		if (paint != null) {
			return new BasicStroke(paint.getStrokeWidth(), paint.getStrokeCap().ordinal(), paint.getStrokeJoin().ordinal());
		}
		return null;
	}
	...
	public void drawImage(LImage img, int left, int top) {
		if (img != null) {
			drawBitmap(img.getBitmap(), left, top);
		}
	}

	public void drawBitmap(Bitmap bit, float left, float top) {
		if (isClose) {
			return;
		}
		if (bit == null) {
		...
		canvas.drawBitmap(bit, left, top, paint);
	}
	...
	public void drawRegion(LImage src, int x_src, int y_src, int width,
		...
		drawRegion(src.getBitmap(), x_src, y_src, width, height, transform, x_dst, y_dst, anchor);
	}
	...
	public void drawBitmap(Bitmap bit, int x, int y, int w, int h) {
		...
		if (width == w && height == h) {		// 无缩放
			drawBitmap(bit, x, y);
			return;
		}
		
		if (LSystem.isOverrunOS21()) {			// 缩放
	...
	public void drawMirrorBitmap(Bitmap bit, int x, int y, boolean filter) {
		...
		if (dst == null) {
			...
			int width = bit.getWidth();
			int height = bit.getHeight();
			dst = Bitmap.createBitmap(bit, 0, 0, width, height, tmp_matrix, filter);
	...
	public void drawBitmap(int[] colors, int x, int y, int width, int height, boolean hasAlpha) {
		if (isClose) {
			return;
		}
		canvas.drawBitmap(colors, 0, width, x, y, width, height, hasAlpha, paint);
	}
	...
	public void drawImage(LImage img, int x, int y, int w, int h, int x1, int y1, int w1, int h1) {
		if (isClose) {
			return;
		}
		if (img != null) {
	...
	public void fillArc(int x, int y, int width, int height, int sa, int ea) {
		...
		if (ea % 360 == 0) {	// 如果扫描360度，则是绘制椭圆
			fillOval(x, y, width, height);
		}
		Paint.Style tmp = paint.getStyle();
		paint.setStyle(Paint.Style.FILL_AND_STROKE);
		
		rectF.set(x, y, x + width, y + height);		// 椭圆的矩形
		canvas.drawArc(rectF, 360 - (sa + ea), ea, true, paint);
//		canvas.drawArc(rectF, sa, ea, true, paint);
	...
	public void fillOval(int x, int y, int width, int height) {
		...
		rectF.set(x, y, x + width, y + height);		// 椭圆的外部边，即矩形四边
		canvas.drawOval(rectF, paint);				// 根据矩形边，绘制椭圆
	}
