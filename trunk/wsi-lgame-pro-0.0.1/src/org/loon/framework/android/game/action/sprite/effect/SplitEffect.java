...
public class SplitEffect extends LObject implements ISprite {
	...
	public SplitEffect(LImage t, RectBox limit, int d) {
		...
		switch (direction) {
		case Config.RIGHT_UP:				// 左右
		case Config.LEFT_DOWN:
			special = true;
		case Config.TLEFT:
		case Config.TRIGHT:
			v1.set(0, 0);
			v2.set(halfWidth, 0);
			break;
		case Config.LEFT_UP:			// 上下
		case Config.RIGHT_DOWN:
			special = true;
		case Config.TUP:
		case Config.TDOWN:
			v1.set(0, 0);
	...
	public void createUI(LGraphics g) {
		...
		if (!complete) {
			...
			case Config.TUP:
			case Config.TDOWN:
				g.drawImage(image, x1, y1, width, halfHeight, 0, 0, width, halfHeight);			// 上
				g.drawImage(image, x2, y2, width, halfHeight, 0, halfHeight, width, height);	// 下
				break;
			case Config.RIGHT_UP:
			case Config.LEFT_DOWN:
			case Config.TLEFT:
			case Config.TRIGHT:
				g.drawImage(image, x1, y1, halfWidth, height, 0, 0, halfWidth, height);			// 左
				g.drawImage(image, x2, y2, halfWidth, height, halfWidth, 0, width, height);		// 右
				break;
			}
