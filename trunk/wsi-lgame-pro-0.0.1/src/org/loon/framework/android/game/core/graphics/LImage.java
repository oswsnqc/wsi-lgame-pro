...
public class LImage implements LRelease {
	...
	public static class LFormatTGA {
		...
		private static final void writePixel(int[] pixels, final byte red, final byte green, final byte blue, final byte alpha, final boolean hasAlpha, final int offset) {
			int pixel;
			if (hasAlpha) {
				pixel = (red & 0xff);
				pixel |= ((green & 0xff) << 8);
		...
		private static int[] readBuffer(InputStream in, int width, int height, int srcBytesPerPixel, boolean acceptAlpha, boolean flipVertically) throws IOException {
		...
		private static void loadUncompressed(byte[] header, State tga, InputStream in, boolean acceptAlpha, boolean flipVertically) throws IOException {
		...
		private static void loadCompressed(byte[] header, State tga, InputStream in, boolean acceptAlpha, boolean flipVertically) throws IOException {
		...
		public static State load(InputStream in, State tga, boolean acceptAlpha, boolean flipVertically) throws IOException {
	...
	public static LImage createImage(byte[] imageData, int imageOffset, int imageLength, boolean transparency) {
		return GraphicsUtils.loadImage(imageData, imageOffset, imageLength, transparency);
	}

	public static LImage createImage(byte[] imageData, int imageOffset, int imageLength) {
		return GraphicsUtils.loadImage(imageData, imageOffset, imageLength, false);
	}
	 ...
	 * @return
	 */
	public static final LImage createRGBImage(int[] rgb, int width, int height, boolean processAlpha) {
		Bitmap bitmap = null;
		try {
	...
	public LImage(String fileName) {
		...
		if (fileName.toLowerCase().lastIndexOf(tgaExtension) != -1) {
			try {
				...
				if (tga != null) {
					bitmap = Bitmap.createBitmap(tga.pixels, tga.width, tga.height, tga.type == 4 ? Bitmap.Config.ARGB_8888 : Bitmap.Config.RGB_565);
					tga.dispose();
					tga = null;
				}
			} catch (IOException e) {
	...
	public int[] getRGB(int pixels[], int offset, int stride, int x, int y, int width, int height) {
		bitmap.getPixels(pixels, offset, stride, x, y, width, height);
		return pixels;
	}
	...
	public LImage getMirrorImage() {
		if (bitmap != null) {
			synchronized (bitmap) {
				LImage image = LImage.createImage(width, height, bitmap.getConfig());
	...
	public boolean isClose() {
		return isClose || bitmap == null || (bitmap != null ? bitmap.isRecycled() : false);
	}
	...
	/**
	 * 释放图片资源，bitmap.recycle()
	 * 
	 * @param remove ： 是否从链表中移除资源， true ： 移除； false ： 不移除
	 */
	...
	public static void disposeAll() {
		if (images.size() > 0) {
			for (LImage img : images) {
				if (img != null) {
					img.dispose(false);		// 不移除，由 images.clear() 集中释放
					img = null;
				}
			}
			images.clear();
