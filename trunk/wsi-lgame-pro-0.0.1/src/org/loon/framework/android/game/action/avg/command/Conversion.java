...
public abstract class Conversion implements Expression {
	 ...
	 * 
	 * @param s	： 比较操作符（字符串形式）
	 * @return	返回 true ： 是操作符； false ：不是操作符
	 */
	...
	public static String updateOperator(String cmd) {		// cmd = set role_01 = "assets/r0.png"
		if (cmd != null && (cmd.startsWith("\"") || cmd.startsWith("'"))) {		// 注释时，直接返回
			return cmd;
		}
		int size = cmd.length();
		StringBuffer strBuffer = new StringBuffer(size * 2);
		char[] chars = cmd.toCharArray();		// 字符串 String 转换成 字符数组 char[]
		boolean notFlag = false;				// 判断是否是注释
		boolean isOperator;						// 判断是否是操作符，如： +， -， *， /， <， >， =， %， ！
		for (int i = 0; i < size; i++) {
			...
			if (chars[i] == ' ') {
				if (i > 0 && chars[i - 1] != ' ') {
					strBuffer.append(FLAG);		// 添加 "@" 符号
				}
			} else {
				isOperator = isOperator(chars[i]);
				if (i > 0) {
					if (isOperator && !isOperator(chars[i - 1])) {
						if (chars[i - 1] != ' ') {
