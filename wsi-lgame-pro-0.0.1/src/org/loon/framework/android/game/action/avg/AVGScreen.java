...
//	}
	 ...
	 * 构造函数，初始化脚本及对话框
	 * 
	 * @param initscript	： 脚本名（包含路径，如 assets/script/s1.txt）
	 * @param initdialog	： 对话框图片的路径
	 */
	...
	/**
	 * 加载完毕后，运行avg线程，即 avgThread.start()
	 */
		...
		if (sleep == 0) {
			...
			try {
				if (scrCG.getBackgroundCG() != null) {
					if (shakeNumber > 0) {
						g.drawImage(scrCG.getBackgroundCG(), shakeNumber / 2 - LSystem.random.nextInt(shakeNumber),
			...
			if (color != null) {
				...
				if (alpha < 1.0) {
					...
					LColor c = g.getColor();
					g.setColor(color.getRed(), color.getGreen(), color.getBlue(), (int) (255 * alpha));
		...
		synchronized (synch) {
			if (command != null && !isClose() && running) {
				for (; isRunning = command.next();) {
					String result = command.doExecute();	// result = mes 播放音乐吧
					if (result == null) {
						continue;
					}
					if (!nextScript(result)) {	// 抽象函数，用户自己实现的脚本命令
						break;
					}
					List<?> commands = Command.splitToList(result, " ");	// commands = [gb, assets/background.png]
					int size = commands.size();
					String cmdFlag = (String) commands.get(0);				// 命令标记， 例如 gb

					String mesFlag = null, orderFlag = null, lastFlag = null;
					if (size == 2) {
						mesFlag = (String) commands.get(1);					// 命令对应的内容标记，如 assets/background.png
					} else if (size == 3) {
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_WAIT)) {				// 命令 "wait"
						scrFlag = true;
						break;
					}
					if (cmdFlag.equalsIgnoreCase(CommandType.L_SNOW)
							|| cmdFlag.equalsIgnoreCase(CommandType.L_RAIN)
							|| cmdFlag.equalsIgnoreCase(CommandType.L_PETAL)) {		// 命令 "snow", “rain", "petal"
						if (sprites != null) {
							boolean flag = false;
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_SNOWSTOP)
							...
							|| cmdFlag.equalsIgnoreCase(CommandType.L_PETALSTOP)) {		// 命令 "snowstop", “rainstop", "petalstop"
						if (sprites != null) {
							ISprite[] ss = sprites.getSprites();
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_PLAY)) {				// 命令 "play"
						playAssetsMusic(mesFlag, false);
						continue;
					}
					if (cmdFlag.equalsIgnoreCase(CommandType.L_PLAYLOOP)) {			// 命令 "playloop"
						playAssetsMusic(mesFlag, true);
						continue;
					}
					if (cmdFlag.equalsIgnoreCase(CommandType.L_PLAYSTOP)) {			// 命令 "playstop"
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_FADEOUT)
							|| cmdFlag.equalsIgnoreCase(CommandType.L_FADEIN)) {	// 命令 "fadeout", "fadein"
						scrFlag = true;
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_SELLEN)) {		// 命令 "selleft"
						if (mesFlag != null) {
							if (NumberUtils.isNan(mesFlag)) {
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_SELTOP)) {		// 命令 "seltop"
						if (mesFlag != null) {
							if (NumberUtils.isNan(mesFlag)) {
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_MESLEN)) {		// 命令 "meslen"
						if (mesFlag != null) {
							if (NumberUtils.isNan(mesFlag)) {
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_MESTOP)) {		// 命令 "mestop"
						if (mesFlag != null) {
							if (NumberUtils.isNan(mesFlag)) {
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_MESLEFT)) {		// 命令 "mesleft"
						if (mesFlag != null) {
							if (NumberUtils.isNan(mesFlag)) {
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_MESCOLOR)) {		// 命令 "mescolor"
						if (mesFlag != null) {
							if (mesFlag.equalsIgnoreCase("red")) {
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_MESSTOP)) {		// 命令 "messtop"
						scrFlag = true;
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_MES)) {		// 命令 "mes"
						if (select.isVisible()) {
							select.setVisible(false);
						}
						scrFlag = true;
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_SELECT)) {		// 命令 "select"
						selectMessage = mesFlag;
						continue;
					}
					if (cmdFlag.equalsIgnoreCase(CommandType.L_SELECTS)) {		// 命令 "selects"
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_SHAKE)) {		// 命令， shake
						shakeNumber = Integer.valueOf(mesFlag).intValue();
						continue;
					}
					if (cmdFlag.equalsIgnoreCase(CommandType.L_CGWAIT)) {		// 命令， cgwait
						scrFlag = false;
						break;
					}
					if (cmdFlag.equalsIgnoreCase(CommandType.L_SLEEP)) {		// 命令， sleep
						sleep = Integer.valueOf(mesFlag).intValue();
						sleepMax = Integer.valueOf(mesFlag).intValue();
						scrFlag = false;
						break;
					}
					if (cmdFlag.equalsIgnoreCase(CommandType.L_FLASH)) {		// 命令， flash
						scrFlag = true;
	 ...
	 * 初始化命令配置文件
	 * 
	 * @param fileName	： 加载的脚本文件名（包含路径，如 assets/script/s1.txt）
	 */
	...
	/**
	 * 初始化 initAVG， 主要包括初始化 initMessageConfig， initSelectConfig， initCommandConfig
	 */
