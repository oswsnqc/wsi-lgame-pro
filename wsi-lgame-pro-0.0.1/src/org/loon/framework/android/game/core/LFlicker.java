...
public final class LFlicker extends android.view.GestureDetector.SimpleOnGestureListener {
	...
	public LFlicker(LFlickerListener listener) {
		...
		try {
			Runnable runnable = new Runnable() {
				public void run() {
					detector = new android.view.GestureDetector(LSystem.getActivity(), LFlicker.this);
				}
			};
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
		...
		int direction = (a ? FLICK_LEFT : FLICK_UP) | (b ? FLICK_RIGHT : FLICK_UP);
