...
public abstract class Screen implements LInput, LRelease {
	...
	/**
	 * 感应的方向， 方向为： NONE, LEFT, RIGHT, UP, DOWN
	 * 
	 * @author homer
	 */
	...
	private int touchButtonPressed = LInput.NO_BUTTON, touchButtonReleased = LInput.NO_BUTTON;		// 触摸
	private int keyButtonPressed = LInput.NO_KEY, keyButtonReleased = LInput.NO_KEY;				// 按键
	...
	private Sprites sprites;	// 精灵集合
	private Desktop desktop;	// 桌面集合

	private Point touch = new Point(0, 0);

	/**
	 * 加载状态， true - 加载完毕； false - 未加载完毕
	 */
	private boolean isLoad;
	private boolean isLock, isClose, isTranslate;
	...
	public void onCreate(int width, int height) {
		...
		this.currentScreen = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
	...
	/**
	 * 初始化时加载的数据，空函数，需要用户自己实现
	 */
	public  abstract void onLoad();

	/**
	 * 初始化加载完毕，空函数，需要用户自己实现
	 * 
	 */
	...
	public void emulatorButtonsVisible(boolean visible) {
		if (LSystem.getSurface2D() != null) {
			try {
				EmulatorButtons es = LSystem.getSurface2D().getEmulatorButtons();
	...
	public void showAndroidAlert(final ClickListener listener,
		...
		if (handler != null) {
			handler.getLGameActivity().showAndroidAlert(listener, title, message);
		}
	}
	...
	public void showAndroidOpenHTML(final ClickListener listener,
		...
		if (handler != null) {
			handler.getLGameActivity().showAndroidOpenHTML(listener, title, url);
		}
	}
	 ...
	 * @return
	 */
	public void showAndroidTextInput(final TextListener listener, final String title, final String text) {
		if (handler != null) {
			handler.getLGameActivity().showAndroidTextInput(listener, title, text);
		}
	}
	 ...
	 * 播放Assets中的音频文件
	 * 
	 * @param fileName	: 播放音乐文件，如 res/music/no_say_goodbye.wav
	 * @param loop	: 是否循环播放， true - 循环； false - 不循环
	 */
	public void playAssetsMusic(final String fileName, final boolean loop) {
		if (handler != null) {
			handler.getAssetsSound().playSound(fileName, loop);
		}
	}
	...
	public final void callEvents(boolean execute) {
		...
		ArrayList<Runnable> runnableList;	// 临时变量
		synchronized (runnables) {
		...
		for (Iterator<Runnable> it = runnableList.iterator(); it.hasNext();) {
			...
			synchronized (running) {
				try {
					if (running instanceof Thread) {		// thread
		...
		try {
			...
			if (MultitouchUtils.isMultitouch()) {
				...
				for (int idx = 0; idx < pointerCount; idx++) {
					...
					touchX = (MultitouchUtils.getX(e, id) - tx) / LSystem.scaleWidth;
					touchY = (MultitouchUtils.getY(e, id) - ty) / LSystem.scaleHeight;
					touchEvent.x = touchX;
					touchEvent.y = touchY;
	...
	public void onSensorChanged(android.hardware.SensorEvent e) {
		...
		synchronized (this) {
			...
			if ((curTime - lastUpdate) > 30) {
				...
				currenForce = Math.abs(currentX + currentY + currentZ - lastX - lastY - lastZ) / diffTime * 10000;
