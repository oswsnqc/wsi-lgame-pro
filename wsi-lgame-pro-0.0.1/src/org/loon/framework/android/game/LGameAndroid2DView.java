...
public class LGameAndroid2DView extends SurfaceView implements SurfaceHolder.Callback {

	final static private DrawFilter zoomFilter = new PaintFlagsDrawFilter(Paint.FILTER_BITMAP_FLAG | Paint.DITHER_FLAG | Paint.SUBPIXEL_TEXT_FLAG, Paint.LINEAR_TEXT_FLAG);
	 ...
	 * CanvasThread 线程实例
	 */
	private CanvasThread gameThread;
	...
	public LGameAndroid2DView(LGameAndroid2DActivity activity, boolean isLandscape, LMode mode) {
		...
		try {
			LSystem.setupHandler(activity, this, isLandscape, mode);	// set LHandler, 传参数至 LSystem
	...
	private void createScreen() {
		...
		if (LSystem.isLowerVer()) {			// 低于版本 1.6
	...
	public void surfaceCreated(SurfaceHolder holder) {
		try {
			if (!isRunning) {
				isRunning = true;
				if (gameThread == null) {
					this.gameThread = new CanvasThread();
					this.drawPriority = Thread.NORM_PRIORITY;
					this.gameThread.setPriority(drawPriority);
	...
	/**
	 * 设置 screen，通过 handler.setScreen(screen) 实现，其中 handler 为 LHandler 实例
	 * 
	...
	/**
	 * 返回当前的 screen，即 currentScreen
	 * 
	 * @return Bitmap的实例 currentScreen
	 */
	public final Bitmap getImage() {
		if (handler != null && !LSystem.AUTO_REPAINT && handler.getRepaintMode() == Screen.SCREEN_NOT_REPAINT) {
			return currentScreen;
		} else {
			if (gameThread != null) {
				gameThread.updateCapture(currentScreen);
			}
			return currentScreen;
		}
	}
	...
	/**
	 * 释放 gameView 资源， 如 停止音效/handler.destroy()/releaseResources()
	 */
	...
	/**
	 * 更新游戏画布， 即 if (LSystem.isLogo) ： canvas.drawBitmap(currentScreen, 0, 0, null)； else gl.update(canvas)
	 * 
	 */
	public void update() {
		try {
	...
	final private class CanvasThread extends Thread {
		...
		private long before, lastTimeMicros, currTimeMicros, goalTimeMicros,
				elapsedTimeMicros, remainderMicros, elapsedTime, frameCount, frames;
			...
			if (LSystem.AUTO_REPAINT) {
				...
				synchronized (surfaceHolder) {
					...
					case Screen.SCREEN_NOT_REPAINT:
						break;
					default:
						gl.drawBitmap(handler.getBackground(), repaintMode / 2 - LSystem.random.nextInt(repaintMode), repaintMode / 2 - LSystem.random.nextInt(repaintMode));
						break;
					}
					...
					if (isMemory) {
						...
						String memory = ((float) ((currentMemory * 10) >> 20) / 10) + " of " + ((float) ((totalMemory * 10) >> 20) / 10) + " MB";
		...
		private void showLogo() {
			...
			try {
				...
				try {
					if (logo == null) {
						logo = GraphicsUtils.loadNotCacheImage(LSystem.FRAMEWORK_IMG_NAME + "logo.png", true);
					}
					cx = LGameAndroid2DView.this.getWidth() / 2 - logo.getWidth() / 2;
					cy = LGameAndroid2DView.this.getHeight() / 2 - logo.getHeight() / 2;
				} catch (Exception e) {
				}
		 ...
		 */
		public void run() {
			if (LSystem.isLogo) {		// 显示 logo
				showLogo();
			}
			boolean isScale = handler.isScale();

			Thread currentThread = Thread.currentThread();	// 当前游戏线程
			int restoreCount = 0;
			try {
				do {
					...
					if (LSystem.AUTO_REPAINT) {
						canvas = surfaceHolder.lockCanvas(null);		// 获取 Canvas
						if (canvas == null) {
							continue;
						}
						synchronized (surfaceHolder) {
							...
							default:
								if (handler.getX() == 0 && handler.getY() == 0) {
									...
									gl.drawClear();
									gl.drawBitmap(handler.getBackground(), handler.getX() + repaintMode / 2 - LSystem.random.nextInt(repaintMode), handler.getY() + repaintMode / 2 - LSystem.random.nextInt(repaintMode));
								}
								break;
							}
				...
				} while (isRunning && gameThread == currentThread);
			} catch (Exception ex) {
	...
	/**
	 * 设置运行线程标记， 即 this.isRunning = isRunning
	 * @param isRunning
	 */
	...
	/**
	 * 释放 surfaceHolder，即 surfaceHolder.removeCallback(this)
	 */
	...
	public void setDrawPriority(int drawPriority) {
		if (drawPriority >= 0 && drawPriority <= 10) {
			...
			if (gameThread != null) {
				try {
					gameThread.setPriority(drawPriority);
				} catch (Exception e) {
				}
			}
		}
